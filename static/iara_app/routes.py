...
def inspection_details(inspection_id):
    # Allow both inspectors (own inspections) and admins (any inspection)
    if current_user.role not in ["inspector", "administrator"]:
        abort(403)

    inspection = Inspection.query.get_or_404(inspection_id)

    # Inspectors can only see their own; admins can see all
...
# ============================================================
# PERMIT STATUS ACTIONS (ADMIN ONLY)
# ============================================================
...
def renew_permit(permit_id):
    ...
    if request.method == "POST":
        ...
        try:
            new_expiry = _date.fromisoformat(new_expiry_str)
        except ValueError:
            flash("Invalid date format.", "danger")
            return redirect(request.url)

        if new_expiry <= _date.today():
